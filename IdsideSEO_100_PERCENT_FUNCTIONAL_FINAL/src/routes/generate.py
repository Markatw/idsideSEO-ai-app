...
# Configure OpenAI API with new format
client = OpenAI(
    ...
    try:
        ...
        for i in range(min(num_variations, 5)):  # Limit to 5 variations max
            try:
                ...
                # Use OpenAI ChatGPT API with new format and supported model
...
Generate only the CTA text, nothing else."""
                ...
                response = client.chat.completions.create(
                    model="gpt-4.1-mini",
                    messages=[
                        {"role": "system", "content": "You are a social media marketing expert specializing in creating compelling calls-to-action."},
                        {"role": "user", "content": prompt}
                    ],
                    max_tokens=100,
                    temperature=0.7
                )
                custom_cta = response.choices[0].message.content.strip()
            except Exception as e:
...
Return only the hashtags, one per line, starting with #"""
        ...
        response = client.chat.completions.create(
            model="gpt-4.1-mini",
            messages=[
                {"role": "system", "content": "You are a social media hashtag expert who creates relevant and engaging hashtags."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=200,
            temperature=0.7
